...
namespace kreg
{
    ...
    public:
        Group(std::string_view _file, std::string_view password, std::string_view manager, std::string_view _host)
            : host(_host)
            , file(_file)
            , pw(password) 
        { 
        ...
        template < typename F > void EnumerateStream(F&& f,std::string _stream ="")
        {
            ...
            auto _pool = HttpConnection(host).Get(std::string("/stream?id=") + stream, std::string_view());
        ...
        void AddElement(std::string description)
        {
            ...
            auto result = HttpConnection(host).Post(std::string("/join_group?id=") + element + "&stream=" + stream + "&desc=" + d8u::util::to_hex(description), std::string_view());
